  ...
  return (
    ...
    >
      {bubbles.map((b) => (
        <motion.div
          key={b.id}
          className={`pointer-events-auto absolute cursor-grab active:cursor-grabbing ${b.top} ${b.right} ${b.hideOnMobile ? "hidden md:flex" : "flex"}`}
          drag
          dragConstraints={containerRef}
          dragElastic={0.2}
        >
          <motion.div
            className="flex h-12 w-12 items-center justify-center rounded-full font-mono text-sm md:h-14 md:w-14 lg:h-16 lg:w-16"
